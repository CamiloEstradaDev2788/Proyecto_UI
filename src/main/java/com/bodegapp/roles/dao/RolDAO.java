package com.bodegapp.roles.dao;
...
public class RolDAO {
    ...
    public List<RolModel> listar(int idEmpresa){
    List<RolModel> lista = new ArrayList<>();
    String SQL = "SELECT * FROM roles WHERE ID_EMPRESA = ?";

    try (Connection con = ConexionBD.getConexion();
         PreparedStatement ps = con.prepareStatement(SQL)) {

        ps.setInt(1, idEmpresa);   // Primero setear el parámetro
        try (ResultSet rs = ps.executeQuery()) {  // Luego ejecutar
            while (rs.next()) {
